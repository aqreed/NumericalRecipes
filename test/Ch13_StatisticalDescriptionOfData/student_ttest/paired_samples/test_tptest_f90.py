"""
    Python interface for the unit tests of the "tptest" Fortran function.
...
path2fortran_test = 'test/Ch13_StatisticalDescriptionOfData/student_ttest/paired_samples/'
...
system('gfortran -c ' + path2Ch13 + 'student_ttest/paired_samples/tptest.f90')
system('gfortran -c test_tptest.f90')
system('gfortran avevar.o betai.o betacf.o gammaln.o \
        tptest.o test_tptest.o -o ftest_tptest')  # Link objects
system('rm *.o *.mod')  # Remove unnecessary objects and module
chdir(owd)


# Fortran wrapper function
def fortran_tptest(data1, data2):
    aux = [str(len(data1))]
    aux += [str('') + str(s) + str('') for s in data1]
    aux.insert(0, owd + '/' + path2fortran_test + 'ftest_tptest')
